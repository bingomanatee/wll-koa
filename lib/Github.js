...
const Github = {
  ...
  dirArticles: (data) => {
    ...
    for (let name of Object.keys(matches)) {
      files.push(Object.assign({ name, dir, parent, path }, matches[name]));
    }
  ...
  loadDirFiles: async (dir) => {
    ...
    if (dir.dirs) {
      promises = promises.concat(dir.dirs.map(Github.loadDirFiles));
    }
  ...
  loadFileBlob: async (file) => {
    ...
    return Promise.all([
      async () => {
        ...
        file.content = contentStr.substr(0, 100);
      },
      async () => {
